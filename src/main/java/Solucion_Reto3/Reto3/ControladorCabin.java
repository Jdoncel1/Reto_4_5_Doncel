...
public class ControladorCabin {
    ...
    @GetMapping("/all")
    public List<Cabañas> getCabin(){
        return servicio.getAll();
    }

    @GetMapping("/{id}")
    public Optional<Cabañas> getCabin(@PathVariable("id") int cabinId) {
        return servicio.getCabin(cabinId);
    }

    @PostMapping("/save")
    @ResponseStatus(HttpStatus.CREATED)
    public Cabañas save(@RequestBody Cabañas cabin) {
        return servicio.save(cabin);
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public Cabañas update(@RequestBody Cabañas cabin) {
